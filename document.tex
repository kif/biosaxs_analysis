...
To cope with the resulting increased data-flux, analysis software were rewritten to ensure real-time processing.
...
\label{subtracted}
\begin{center}
\includegraphics[width=12cm, height=10cm]{Figure_1.png}
...
Everything is executed on a single computer, single processor (6 cores) with an entry-level graphics card. During those 20 month, the \textit{dahu}-server has been started 90 times, which corresponds to the weekly restart to apply security patches and other bug-corrections. 
Since all data-reduction occurs within the same process using threads, this proves not only the reliability of the \textit{dahu}-server but also of the whole pipeline including FreeSAS analysis.
\subsection{outlook}
...
This document introduces the \textit{FreeSAS} and the \textit{dahu} software packages which are used respectively to analyse bio-SAS data and control online data analysis at the BioSAXS beamline at the European synchrotrom. 
Those two packages are combined with others to provide complete data analysis pipelines.
The three pipelines described in this contribution are used in production since 2020, and provide real-time feed-back of ongoing experiment to the user.
All metadata, all parameters and all references to the source data are recorded together with the processed data into a single HDF5 files which offers not only convenient storage but allows also reproducible science in the FAIR principle. 
