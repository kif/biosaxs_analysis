...
EDNA was processing data frame per frame and used to take 2.3s to integrate a 1 mega-pixel image from the former Pilatus detector.
With this new pipeline, 1s are usually needed to integrate ten 2.4 mega-pixel images in sample-changer mode and 10s for the 100 frames per file in \textsc{hplc} mode, (table \ref{stats}).

All computations are now executed on a single computer equipped with a single hexa-core processor and an entry-level graphics card (Intel Xeon E5-2643 v3 + Nvidia Quadro M2000) which is nevertheless much more recent than the previous se.
