...
To meet the real-time feed-back requirements, the software doing the analysis was also re-written.
...
The \textit{dahu} job-manager was designed for the TruSAXS beamline \cite{id02_2022} with those limitation in mind and a short latency constrain: some jobs, there, needs to be processed with a couple of milliseconds. 
This latency constrain prevented us from using classical batch queuing systems like \textsc{slurm} \cite{slurm}, despite their efficiency at distributing heavy jobs, since their scheduling algorithms take several seconds to run.
...
This data-structure with several frame per file prevents us from re-using the former \textsc{edna} pipelines which were  all triggered frame by frame.
...
With this new pipeline, 1 s are usually needed to integrate ten 2.4 mega-pixel images in sample-changer mode and 10 s for the 100 frames is a  file in \textsc{hplc} mode, (table \ref{stats}), which makes average throughput of 10 frames/s.

Performances for the \texit{sample-changer pipeline} (7.3 s) can directly be compared with the former EDNA processing which were called \textit{autoSub} (2.1 s) and \texit{SAXSAnalysis} (9.7s).
