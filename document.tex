...
manages the execution identifier, exposes the status and controls the execution of the underlying \textit{plugin}. %via the \textit{setup-process-teardown} sequence.
...
The BioSAXS beamline uses pyFAI \cite{pyfai_2020} as core of its \textit{multiframe} integration pipeline implemented in \textit{dahu}.
...
Frames are read from the HDF5 file produced by \textit{LImA} \cite{lima} and all other metadata like the sample description and the transmitted  intensities (recorded by the beam-stop diode) are sent as part of the job description from BsxCuBE3, the graphical user interface running the beamline.
...
\includegraphics{multiframe.png}
\end{figure}
...
For this, every single scattering curve is compared with all others within a file to build the correlation-map \cite{cormap}. 
This allows to determine the largest group of frames which are equivalent based on two threshold, the minimum probability for two adjacent frames 
